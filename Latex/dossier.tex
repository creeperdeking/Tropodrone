...
Plusieurs thèses étudiant la perméabilité des polymères au gaz comme l'hélium on était trouvé mais la manque de valeurs pour l'épaisseur de notre filme, la pression utilisée ou encore la température ont rendu l'usage de ces formules erroné.
...
Les seul chiffres que nous possédons sont ceux d'un film de PET alimentaires le Mylar 850 déjà énoncé précédemment~:
...
\begin{figure}[H]
	...
	\caption{Test d'étanchéité du PET, le PET est maintenu par la cloche dans l'eau, nous mesurons la chute de la masse disposé au dessus du PET.}
\end{figure}
...
De plus les données constructeur de la colle indiquent une résistance de la colle en traction de 7 à 11 $N.mm-2$ ce qui largement supérieur au .12 $N.mm-2$ des tests, ce qui laisse penser que seul le PET et limitant dans la résistance des collages maintenant.


Cette technique de collage sera retenue pour l'assemblage de tout les futur ballons.
...
Les 4 moteurs du drone lui confèrent 3 axes de liberté, une de nos contraintes était de permettre de sauvegarder ces libertés pour perdre un minimum de liberté de mouvement, pour celà nous avons choisis une structure triangulaire. Dans cette structure, le centre de gravité du drone est le même que le centre de gravité du ballon, ce qui permet de controller le tropodrone dans toutes les directions. Cette structure permet d'optimiser le nombre de ballons (3) nécessaires pour avoir une pénétration uniforme dans l'air quel que soit la direction prise par le drone.
...
[PHOTO]
Toutefois, ce modèle s'est avérré ne pas être applicable car il était très complexe à mettre en oeuvre, nécessitant une modélisation de pièces en 3D complexes, devant en plus être retravaillés après impression et l'ajout de petits roulement pour résoudre des problèmes de frottements des pièces en plastiques. Celà aurait augmenté le cout du produit final, sa fragilité et sa complexité. De plus le nombre de pièces à créer rendait la structure trop lourde.
...
Le drone est suspendu par le haut et par le bas par un fil de pèche au centre d'un octaèdre inscrit dans la structure triangulaire des ballons. Cette solution présente les avantages suivants:
	- Simplicité
	- Economie, cette solution est moins chère
	- Légerté
...
Conaissant les contraintes du projet, nous avons optés pour des tiges en carbonne, car elles étaient facile à trouver et présentaient un très bon rapport solitité/poids, face aux différentes attaches en métal par exemple. Malheureuxement, elles coutaient assez cher (4€ le mètre en moyenne), mais nous avions besoins d'une structure à la fois flexible (pour résister au choc) et rigide (pour le triangle principale, pour éviter les collisionsentre le drone et la structure). En jouant sur les tailles des tiges et les versions creuses/pleines, nous avons obtenus le matériel de base de notre structure.
...
\iffalse
	Nous avons également pû relever la force théorique maximale auquel le drone peut être soumis, qui est de
	Chaque moteur supporte 4.3 N
